...
**Changed**
- **Nightly builds run when the module changes, not on a timer.** The workflow fired on every push to `main` — a README fix, a CHANGELOG entry or the `update.json` commit a release pushes all spent a full nine-variant run — and again every night on an unchanged tree. It now triggers on the paths a zip is actually made of (and on pull requests touching them), the way the rest of the TrickyStore family gates its builds; none of them builds on a schedule either. Artifacts are kept for 90 days instead of 30, so the nightly.link URLs survive a quiet stretch on `main`.
- **Every language now covers the whole WebUI.** Only English and Turkish had the Advanced tab, the target-app picker, the custom-keybox card, the theme picker and the toasts; the other thirteen (Russian, Spanish, Portuguese, German, French, Italian, Indonesian, Vietnamese, Arabic, Chinese, Japanese, Korean, Hindi) fell back to English for all of it. All fifteen are complete now.
- **`/data/adb/tricky_store/` is the engine's directory again.** Every switch in the WebUI used to be its own file there (`no_auto_fp`, `no_auto_keybox`, `no_rom_spoof_block`, `hide_rom_markers`, `hourly_interval_sec`, …), next to our per-app map, our logs, the engine loop pids and every half-finished download — two dozen entries mixed in with `keybox.xml`, `target.txt`, `hbk` and `persistent_keys`. All of it now lives in one directory, `alwaysstrong/`, with one `config` file holding every setting as a `key=value` line (`auto_fp=0`, `interval_sec=1800`), plus `state`, `apps.map`, `packages`, `spoof.conf`, `logs/` and `tmp/`. Settings are named for what they do rather than what they switch off, and the file is written out in full — every setting with its current value — on install, on boot and on reset, so you can read or edit the lot in one place instead of discovering a setting only once you have touched it. Your existing settings are imported on install — nothing to redo — and the old files are cleared away.
...
- **Nightly artifacts say they are nightlies.** A CI build is made from whatever is on `main`, but it shipped the plain release version, so a tester's build was indistinguishable from the released one in the manager and in bug reports. Nightlies are now tagged the way the rest of the TrickyStore family tags its builds — TrickyStore ships `v1.4.1 (245-72b2e84-release)`, TEESimulator `v4.0 (63-123d8ba-release)` — with `nightly` as the build type: `v1.0.4 (411-06f040e-nightly)` in the manager, in the WebUI chip and in a collected log, and `AlwaysStrong-v1.0.4-411-06f040e-nightly.zip` for the zip inside the artifact. `versionCode` stays the release's (upstream puts the commit count there; ours tracks the release), so the next release still shows up as an update. Local builds can do the same with `build.sh --build-tag`.
...
**Fixes**
- **A shorter check interval took effect an hour later.** The background refresh slept the whole interval in one go, with the value read once at the start of that sleep — turning 60 minutes into 1 in the WebUI still sat out the rest of the hour, so the change looked ignored. Writing the setting now tells the loop, which drops the wait it is in and starts the new interval from that moment; the value is still read once per wait and nothing is polled, so an idle hour costs the single wakeup it always did. This is how nginx treats a changed config — the daemon does not watch the file, `nginx -s reload` tells it — and `sh as_store.sh reload` does the same here for a config edited by hand outside the module (without it, a hand edit lands on the next cycle). Verified on device: the loop restarted within seconds of the change, and the keybox deleted mid-hour came back on the next run.
- **App target list: tapping a row did nothing.** Selecting or deselecting an app was decided by whether the long-press timer was still pending, and any pointer movement cancelled that timer — a finger on glass always drifts a pixel or two, so on most devices the tap was thrown away with it and the whole list looked inert, leaving no way to exclude an app except editing `app_keybox.map` by hand. A tap now only has to stay within 12 px, the long-press (per-app keybox / mode) is unaffected, and a WebView without pointer events falls back to plain clicks.
- **Long-pressing an app row opened the WebView's Copy / Share bar over the dialog.** The per-app keybox and mode dialog did open, but the same press started a text selection underneath it, so selection handles and the system Copy / Paylas / Select-all bar landed on top of the buttons. The WebUI is a control panel, not a document: text selection is off everywhere except the text fields.
