...
**Changed**
- **`/data/adb/tricky_store/` is the engine's directory again.** Every switch in the WebUI used to be its own file there (`no_auto_fp`, `no_auto_keybox`, `no_rom_spoof_block`, `hide_rom_markers`, `hourly_interval_sec`, …), next to our per-app map, our logs, the engine loop pids and every half-finished download — two dozen entries mixed in with `keybox.xml`, `target.txt`, `hbk` and `persistent_keys`. All of it now lives in one directory, `alwaysstrong/`, with one `config` file holding every setting as a `key=value` line (`auto_fp=0`, `interval_sec=1800`), plus `state`, `apps.map`, `packages`, `spoof.conf`, `logs/` and `tmp/`. Settings are named for what they do rather than what they switch off, a setting at its default costs nothing on disk, and deleting `config` is exactly "reset to defaults". Your existing settings are imported on install — nothing to redo — and the old files are cleared away.
